...
int main() {
	...
	*/
	...
	while (1) { // 사용자에게 계속 입력받기 위해 무한 루프
		...
		if (user_input == "up") {
		    movePlayer(map, user_x, user_y, 0, -1, mapX, mapY, HP);  // 위로 이동
		}
		else if (user_input == "down") {
		    movePlayer(map, user_x, user_y, 0, 1, mapX, mapY, HP);    // 아래로 이동
		}
		else if (user_input == "left") {
		    movePlayer(map, user_x, user_y, -1, 0, mapX, mapY, HP);   // 왼쪽으로 이동
		}
		else if (user_input == "right") {
		    movePlayer(map, user_x, user_y, 1, 0, mapX, mapY, HP);    // 오른쪽으로 이동
		}
		...
		checkItem(map, user_x, user_y, HP);
